...
public interface OutboxEventRepository extends JpaRepository<OutboxEvent, Long> {
    ...
    @Query("""
            ...
            where event.id = :id
              and event.status = com.smartexpense.expenseservice.entity.OutboxEventStatus.PENDING
              and event.nextAttemptAt <= :now
    ...
    @Query("""
            update OutboxEvent event
            set event.status = com.smartexpense.expenseservice.entity.OutboxEventStatus.PUBLISHED,
    ...
    @Query("""
            update OutboxEvent event
            set event.status = com.smartexpense.expenseservice.entity.OutboxEventStatus.FAILED,
                event.lastError = :lastError
            where event.id = :id
              and event.status = com.smartexpense.expenseservice.entity.OutboxEventStatus.PENDING
            """)
